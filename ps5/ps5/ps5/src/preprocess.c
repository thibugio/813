    ...
    while (line_read != NULL) {
        if (!(line_read[0] == '\n' || line_read[0] == '\r' || line_read[0] == '#')) {
            ...
            for (i=0;;i++) {
                ...
                if (tok == NULL) break;
                if (i==0) {
                    if (!my_stricmp("input", tok)) {  
                        g->gate_type = tinput;
                        g->level = 0;
                    } else if (!my_stricmp("output", tok)) {
                        g->gate_type = toutput;
                        g->output = true;
                    } else {
                    ...
                    if (primary) {
                        ...
                        if (!my_stricmp("dff", tok)) {
                            g->gate_type = tdff;
                            g->level = 0;
                        } else if (!my_stricmp("not", tok))
                            g->gate_type = tnot;
                        else if (!my_stricmp("and", tok))
                            g->gate_type = tand;
                        else if (!my_stricmp("nand", tok))
                            g->gate_type = tnand;
                        else if (!my_stricmp("or", tok))
                            g->gate_type = tor;
                        else if (!my_stricmp("nor", tok))
                            g->gate_type = tnor;
                        else if (!my_stricmp("xor", tok))
                            g->gate_type = txor;
                        else if (!my_stricmp("xnor", tok))
                            g->gate_type = txnor;
                    }
                } else { // i > 1
...
bool has_cycle(struct hashtable* ht) {
    ...
    struct binaryheap* bh = binaryheap_create_from_array(gate_count, gates, gate_count);
...
void stupid_print_gates_to_file(FILE* fd, struct gate* gates[], unsigned int* fanin_ids[], unsigned int* fanout_ids[], unsigned int ngates) {
    if (fd == NULL) return;
    unsigned int i, j, k;
    struct gate* g;
    fprintf(fd, "%u\n", ngates);
    for (i=0; i<ngates; i++) {
        j = 0;
        while (j < ngates && gates[j]->id != i)
            j++; 
        if (j == ngates) {  
            printf("gate id not found\n");
            return;
        }
        ...
        fprintf(fd, "%d %d ", g->level, g->nfanin);
        for (k = 0; k < g->nfanin; k++)
            fprintf(fd, "%u ", fanin_ids[j][k]);

        fprintf(fd, "%d ", g->nfanout);
        for (k = 0; k < g->nfanout; k++)    
            fprintf(fd, "%u ", fanout_ids[j][k]); 
...
int main(int argc, char* argv[]) {
    ...
    struct hashtable* ht = hashtable_create(filesize, 0.6);
    ...
    // fix gate ids and fan lists
    unsigned int gate_count = ht->size;
    struct gate* gates[gate_count];
    hashtable_squeeze_values(ht, (void**)gates);

    unsigned int* fanin_ids[gate_count];
    unsigned int* fanout_ids[gate_count];
    
    unsigned int i, j;
    for (i = 0; i < gate_count; i++) {
        if (gates[i]->nfanin > 0) {
            fanin_ids[i] = malloc(sizeof(unsigned int) * gates[i]->nfanin);
            if (fanin_ids[i] == NULL) {
                printf("ERROR! memory could not be allocated.\n");
                exit(EXIT_FAILURE);
            }
            struct gatelistnode* f = gates[i]->fanin;
            struct gate* temp;
            j = 0;
            while (f != NULL) {
                temp = hashtable_find(ht, f->name);
                fanin_ids[i][j] = temp->id;
                f = f->next;
                j++;
            }
        }
        if (gates[i]->nfanout > 0) {
            fanout_ids[i] = malloc(sizeof(unsigned int) * gates[i]->nfanout);
            if (fanout_ids[i] == NULL) {
                printf("ERROR! memory could not be allocated.\n");
                exit(EXIT_FAILURE);
            }
            struct gatelistnode* f = gates[i]->fanout;
            struct gate* temp;
            j = 0;
            while (f != NULL) {
                temp = hashtable_find(ht, f->name);
                fanout_ids[i][j] = temp->id;
                f = f->next;
                j++;
            }
        }
    ...
    // n^2...
    stupid_print_gates_to_file(fout, gates, fanin_ids, fanout_ids, gate_count);
